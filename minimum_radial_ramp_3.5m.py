...
For a 3.5m drop (instead of 2.8m), we need a larger radius to maintain
safe vertical curvature.
"""
...
# Ramp requirements - NEW: 3.5m drop
VERTICAL_DROP = 3.5  # m (increased from 2.8m)
...
def find_minimum_radius():
    ...
    for R in np.arange(5.0, 15.0, 0.25):
...
def create_minimum_ramp_design(result):
    ...
    print(f"\n{'=' * 80}")
    print(f"MINIMUM RADIAL RAMP DESIGN - {VERTICAL_DROP}m DROP")
    print(f"{'=' * 80}")
...
def generate_construction_data(design):
    ...
    # Save to CSV
    import csv
    with open('/workspaces/RAMP/minimum_radial_3.5m_measurements.csv', 'w', newline='') as f:
        writer = csv.writer(f)
    ...
    print(f"\nMeasurements saved to: minimum_radial_3.5m_measurements.csv")
...
def create_visualization(design):
    ...
    ax1.set_title(f'TOP VIEW (Plan)\nMinimum Radius for {VERTICAL_DROP}m Drop', fontsize=12, fontweight='bold')
    ...
    ax2.fill_between(s, z, -4.5, color='#d4a574', alpha=0.7)
    ...
    for dist in np.arange(0, arc_length + 0.25, 0.5):
        if dist <= arc_length:
            ...
            if int(dist * 2) % 2 == 0:
                ax2.text(dist, depth - 0.15, f'{abs(depth)*100:.0f}', fontsize=7,
                        ha='center', va='top', rotation=45, color='darkred')
            else:
                ax2.text(dist, depth + 0.10, f'{abs(depth)*100:.0f}', fontsize=6,
    ...
    ax2.set_title(f'SIDE ELEVATION - {VERTICAL_DROP}m Drop - Measurements every 50cm', fontsize=12, fontweight='bold')
    ...
    specs_text = f"""
╔═════════════════════════════════════════════╗
║  MINIMUM RADIAL RAMP - {VERTICAL_DROP}m DROP           ║
╠═════════════════════════════════════════════╣
║  GEOMETRY                                   ║
...
╚═════════════════════════════════════════════╝
    ...
    table_header = f"ELEVATION PROFILE (every 50cm)\n"
    table_header += f"Drop: {VERTICAL_DROP}m\n"
    table_header += "═" * 36 + "\n"
    ...
    full_table = f"DETAILED CONSTRUCTION MEASUREMENTS - {VERTICAL_DROP}m DROP (Centerline coordinates every 50cm)\n"
    full_table += "═" * 120 + "\n"
    ...
    plt.suptitle(f'MINIMUM RADIAL RAMP - {VERTICAL_DROP}m DROP\n'
                 f'R={R:.2f}m Centerline - Arc Length {arc_length:.2f}m - Width {ramp_width:.2f}m',
                 fontsize=14, fontweight='bold', y=0.98)

    plt.savefig('/workspaces/RAMP/minimum_radial_3.5m_blueprint.png', dpi=200, bbox_inches='tight')
    print(f"\nBlueprint saved: minimum_radial_3.5m_blueprint.png")
...
def create_comparison_chart():
    """Create comparison between 2.8m and 3.5m drops."""

    fig, axes = plt.subplots(2, 2, figsize=(14, 10))

    drops = [2.8, 3.5]
    colors = ['blue', 'red']
    labels = ['2.8m drop', '3.5m drop']

    R_min_vert = calculate_minimum_vertical_radius()

    # Plot 1: Vertical radius vs centerline radius
    ax1 = axes[0, 0]
    for drop, color, label in zip(drops, colors, labels):
        radii = np.arange(5.0, 15.0, 0.25)
        vert_radii = []
        for R in radii:
            vert = calculate_vertical_curvature(R, drop)
            vert_radii.append(vert['min_vertical_radius'])
        ax1.plot(radii, vert_radii, color=color, linewidth=2, label=label)

    ax1.axhline(y=R_min_vert, color='gray', linestyle='--', linewidth=2, label=f'Min required ({R_min_vert:.2f}m)')
    ax1.set_xlabel('Centerline Radius (m)')
    ax1.set_ylabel('Min Vertical Radius (m)')
    ax1.set_title('Vertical Curvature vs Radius')
    ax1.legend()
    ax1.grid(True, alpha=0.3)
    ax1.set_ylim(0, 25)

    # Plot 2: Max slope vs centerline radius
    ax2 = axes[0, 1]
    for drop, color, label in zip(drops, colors, labels):
        radii = np.arange(5.0, 15.0, 0.25)
        slopes = []
        for R in radii:
            vert = calculate_vertical_curvature(R, drop)
            slopes.append(vert['max_slope'])
        ax2.plot(radii, slopes, color=color, linewidth=2, label=label)

    ax2.set_xlabel('Centerline Radius (m)')
    ax2.set_ylabel('Maximum Slope (°)')
    ax2.set_title('Maximum Slope vs Radius')
    ax2.legend()
    ax2.grid(True, alpha=0.3)

    # Plot 3: Arc length vs radius
    ax3 = axes[1, 0]
    radii = np.arange(5.0, 15.0, 0.25)
    arc_lengths = [np.pi * R / 2 for R in radii]
    ax3.plot(radii, arc_lengths, 'b-', linewidth=2)
    ax3.axhline(y=12, color='gray', linestyle='--', label='Straight 12m reference')
    ax3.set_xlabel('Centerline Radius (m)')
    ax3.set_ylabel('Arc Length (m)')
    ax3.set_title('Arc Length vs Radius')
    ax3.legend()
    ax3.grid(True, alpha=0.3)
    ...
    # Find minimum for each drop
    min_2_8 = None
    min_3_5 = None

    for R in np.arange(5.0, 15.0, 0.25):
        vert_2_8 = calculate_vertical_curvature(R, 2.8)
        vert_3_5 = calculate_vertical_curvature(R, 3.5)

        if min_2_8 is None and vert_2_8['min_vertical_radius'] >= R_min_vert:
            min_2_8 = {'R': R, 'arc': vert_2_8['arc_length'], 'slope': vert_2_8['max_slope']}
        if min_3_5 is None and vert_3_5['min_vertical_radius'] >= R_min_vert:
            min_3_5 = {'R': R, 'arc': vert_3_5['arc_length'], 'slope': vert_3_5['max_slope']}

    comparison = f"""
    COMPARISON: 2.8m vs 3.5m DROP
    ══════════════════════════════════════════════

    Parameter           2.8m Drop      3.5m Drop      Diff
    ─────────────────────────────────────────────────────
    Min centerline R    {min_2_8['R']:.2f}m         {min_3_5['R']:.2f}m         +{min_3_5['R'] - min_2_8['R']:.2f}m
    Arc length          {min_2_8['arc']:.2f}m        {min_3_5['arc']:.2f}m        +{min_3_5['arc'] - min_2_8['arc']:.2f}m
    Max slope           {min_2_8['slope']:.1f}°          {min_3_5['slope']:.1f}°          +{min_3_5['slope'] - min_2_8['slope']:.1f}°

    The 3.5m drop requires:
    • {min_3_5['R'] - min_2_8['R']:.2f}m larger centerline radius
    • {min_3_5['arc'] - min_2_8['arc']:.2f}m longer arc length
    • {min_3_5['slope'] - min_2_8['slope']:.1f}° steeper maximum slope
    """

    ax4.text(0.05, 0.95, comparison, transform=ax4.transAxes,
             fontsize=11, fontfamily='monospace', verticalalignment='top',
             bbox=dict(boxstyle='round', facecolor='lightyellow', alpha=0.9))

    plt.suptitle('Comparison: 2.8m vs 3.5m Vertical Drop\nHow drop height affects minimum radius requirements',
                 fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig('/workspaces/RAMP/drop_comparison_3.5m.png', dpi=150, bbox_inches='tight')
    print("\nComparison chart saved: drop_comparison_3.5m.png")
...
def main():
    ...
    # Final summary
    print(f"\n{'=' * 80}")
    print(f"SUMMARY - MINIMUM RADIAL RAMP FOR {VERTICAL_DROP}m DROP")
    print(f"{'=' * 80}")
    print(f"""
    ...
    COMPARISON TO 2.8m DROP (min R=6.75m):
    ─────────────────────────────────────────
    Centerline: {design['R_centerline']:.2f}m vs 6.75m (+{design['R_centerline'] - 6.75:.2f}m)
    Arc length: {design['arc_length']:.2f}m vs 10.60m (+{design['arc_length'] - 10.60:.2f}m)
    Max slope:  {design['vert']['max_slope']:.1f}° vs 21.6°

    OUTPUT FILES:
    ─────────────────────────────────────────
    • minimum_radial_3.5m_blueprint.png    - Construction blueprint
    • minimum_radial_3.5m_measurements.csv - Detailed measurements
    • drop_comparison_3.5m.png             - 2.8m vs 3.5m comparison
    """)
